...
function renderMedia(
  ...
  const isVideo =
    mediaType === "video" ||
    effectiveUrl.includes("youtube") ||
    effectiveUrl.includes("youtu.be");

  if (isVideo) {
    // Check if it's a YouTube URL
    const youtubeMatch = effectiveUrl.match(
...
export default function Lookbook() {
  ...
  const getCollectionImage = (
    ...
    return (
      products.find(
        (product) => product.collectionId === collectionId && product.image,
      )?.image || fallback
    );
  };
    ...
    <div ref={ref} className="bg-black text-white">
      <div className="h-screen flex items-center justify-center sticky top-0 overflow-hidden">
        <div className="absolute inset-0 opacity-60">
          {renderMedia(
            branding.lookbookMediaUrl,
            lookbookVideo,
            branding.lookbookMediaType,
          )}
        </div>
      ...
      {collections.map((collection, i) => (
        <div
          key={collection.id}
          className="min-h-screen flex items-center justify-center relative py-24 border-t border-white/10 bg-black"
        >
          <div className="container mx-auto px-6 grid grid-cols-1 md:grid-cols-2 gap-24 items-center">
            ...
            >
              <span className="font-mono text-xs text-white/50 uppercase tracking-widest mb-4 block">
                Coleção {i + 1}
              </span>
              ...
              <p className="text-xl font-light text-white/70 leading-relaxed mb-12 max-w-md">
                {collection.description} Uma exploração de formas, texturas e a
                beleza intrínseca dos materiais preciosos.
              </p>
            </motion.div>
      ...
      <div className="h-[50vh] flex items-center justify-center bg-white text-black">
        <div className="text-center">
          ...
          <a
            href="/shop"
            className="inline-block border-b border-black pb-1 font-mono text-sm uppercase tracking-widest hover:text-black/60 transition-colors"
          >
            Ir para a Loja
          </a>
        </div>
